...
import { Camera, History, ShieldCheck, AlertCircle, X, RefreshCw, Smartphone, CheckCircle2 } from 'lucide-react';

const App: React.FC = () => {
  const [products, setProducts] = useState<Map<string, TobaccoProduct>>(new Map());
  ...
  useEffect(() => {
    ...
    if (savedHistory) {
      try {
        ...
        setHistory(parsedHistory.map((h: any) => ({
          ...h,
  ...
  const startScanner = async () => {
    ...
    setTimeout(() => {
      const html5QrCode = new (window as any).Html5Qrcode("scanner-container");
    ...
    <div className="min-h-screen flex flex-col max-w-lg mx-auto bg-white shadow-2xl overflow-hidden relative border-x border-slate-200 font-sans">
      ...
      <main className="flex-1 p-6 flex flex-col gap-6 overflow-y-auto pb-32 bg-slate-50/50">
        ...
        {status === AppStatus.IDLE && (
          <div className="flex flex-col items-center justify-center h-full py-12 gap-8 text-center animate-in fade-in zoom-in-95 duration-700">
        ...
        {status === AppStatus.RESULT && lastResult && (
          <div className={`rounded-[2.5rem] border-2 p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-500 ${
        ...
        {cameraError && (
          <div className="bg-rose-50 border border-rose-100 p-5 rounded-2xl flex gap-4 text-rose-900 animate-in shake">
        ...
        <button
          ...
          className={`w-full py-5 rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3 pointer-events-auto ${
            status === AppStatus.SCANNING 
              ...
              : 'bg-[#006b3f] text-white shadow-emerald-200'
          }`}
        >
          {status === AppStatus.SCANNING ? (
            <>
